 ...
 * 
 * Modified to have some functions public & static for use in texture.cs
 */
...
namespace KUtility {
	public class DDSImage {
		...
		#region DXT1
		public static Bitmap UncompressDXT1(byte[] data, int w, int h) {
		...
		#endregion
		#region DXT5
		public static Bitmap UncompressDXT5(byte[] data, int w, int h) {
			Bitmap res = new Bitmap((w < 4) ? 4 : w, (h < 4) ? 4 : h);
			using (MemoryStream ms = new MemoryStream(data)) {
				using (BinaryReader r = new BinaryReader(ms)) {
					for (int j = 0; j < h; j += 4) {
		...
		#region V8U8
		public static Bitmap UncompressV8U8(byte[] data, int w, int h) {
		...
		public static Bitmap readLinearImage(byte[] data, int w, int h) {
			Bitmap res = new Bitmap(w, h);
			using (MemoryStream ms = new MemoryStream(data)) {
				using (BinaryReader r = new BinaryReader(ms)) {
					for (int y = 0; y < h; y++) {
